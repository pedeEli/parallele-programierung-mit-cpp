...
std::pair<double, double> minmax(std::size_t n, const double* x) {
...
int main(int argc, char* argv[]) {
  ...
  auto t2 = now();
  auto [min_value, max_value] = minmax(len, &x[0]);
  auto t3 = now();

  std::cout << "minimal value: " << min_value << std::endl;
  std::cout << "maximal value: " << max_value << std::endl;
  ...
  std::cout << "time required for minmax:              " << elapsed_time_in_seconds(t2, t3) << " s"  << std::endl;
}